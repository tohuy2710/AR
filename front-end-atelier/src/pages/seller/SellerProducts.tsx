...
import { Sofa, Edit, Trash2, Plus, Star, Layers, Package, Tag } from 'lucide-react';
...
export default function SellerProducts() {
  ...
  const handleDelete = (id: string, name: string) => {
    if (confirm(`Are you sure you want to permanently delete design catalog: ${name}?`)) {
      deleteProduct(id);
    ...
    <div className="font-sans text-stone-900 text-left space-y-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 border-b border-stone-200 pb-5">
        <div className="space-y-1">
          <h1 className="text-2xl font-display font-light text-stone-950 tracking-tight">Artisan Catalog Registry</h1>
          <p className="text-xs text-stone-400">Add, edit, or discontinue premium workshop furniture models.</p>
        </div>

        <Link
          to="/seller/products/new"
          className="px-5 py-3 bg-stone-950 hover:bg-stone-900 text-white rounded-xl text-xs font-mono font-bold uppercase tracking-widest flex items-center gap-2 transition-all shadow-sm self-start sm:self-auto cursor-pointer"
        >
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs text-left text-stone-500 font-sans min-w-[700px]">
              <thead className="text-[10px] font-mono uppercase text-stone-400 bg-stone-50 border-b border-stone-150">
                <tr>
                  <th scope="col" className="px-6 py-4">Showroom Item</th>
                  <th scope="col" className="px-6 py-4">Department</th>
                  <th scope="col" className="px-6 py-4">Lumber / Materials</th>
                  <th scope="col" className="px-6 py-4">Price</th>
                  <th scope="col" className="px-6 py-4">Chamber Score</th>
                ...
                {products.map(p => (
                  <tr key={p.id} className="hover:bg-stone-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4.5 max-w-[200px] truncate">
                      <p className="text-stone-700 text-xs font-light truncate" title={p.materials.join(', ')}>
                        {p.materials.join(', ')}
                      </p>
                    </td>
