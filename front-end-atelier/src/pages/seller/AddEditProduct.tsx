...
export default function AddEditProduct() {
  ...
  const [detailDimensions, setDetailDimensions] = useState(
    "W 100cm x D 100cm x H 75cm",
  );
  const [detailCare, setDetailCare] = useState(
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    if (materials.length === 0 || colors.length === 0 || images.length === 0) {
      alert(
        "Please include at least 1 Sourced Material, 1 Shader, and 1 Image URL asset.",
      );
      return;
    }
    ...
    const compiledProduct: Product = {
      id:
        isEdit && existingProduct
          ? existingProduct.id
          : `prd-${name.toLowerCase().replace(/[^a-z0-9]+/g, "-")}-${Date.now()}`,
      name,
      shortDescription,
      description,
      price,
      ...
      threeSixtyImages:
        isEdit && existingProduct
          ? existingProduct.threeSixtyImages
      ...
      isFeatured:
        isEdit && existingProduct ? existingProduct.isFeatured : false,
    };
  ...
  return (
    <div className="font-sans text-stone-900 text-left space-y-6 sm:space-y-8 max-w-4xl w-full">
      {/* Navigation back */}
      <button
      ...
      <div>
        <h1 className="text-xl sm:text-2xl font-display font-light text-stone-950 tracking-tight">
          {isEdit
            ? "Revise Design Blueprint"
            : "Launch New Architectural Blueprint"}
        </h1>
        <p className="text-xs text-stone-400 mt-1">
          Complete structural coordinate definitions, wood grade metrics, and
          design provenance to list this item.
        </p>
      </div>
      ...
      >
        ...
        <div className="md:col-span-2 bg-white border border-stone-200 p-5 sm:p-8 rounded-2xl sm:rounded-3xl space-y-6 min-w-0">
          <h3 className="text-xs font-mono text-[#B39D69] uppercase font-bold tracking-wider">
            General Information
          </h3>
          ...
          <h3 className="text-xs font-mono text-[#B39D69] uppercase font-bold tracking-wider pt-4 border-t border-stone-100">
          ...
          <div className="space-y-1.5">
            <label className="text-[10px] font-mono text-stone-500 uppercase">
              Preservation & Care Instructions
            </label>
            <input
              required
              type="text"
        ...
        <div className="md:col-span-1 space-y-6">
          ...
          <div className="bg-white p-5 sm:p-6 border border-stone-200 rounded-2xl sm:rounded-3xl space-y-4">
            <h4 className="text-[11px] font-mono font-bold tracking-widest text-stone-400 uppercase">
              Sourced Materials
            </h4>
            ...
            <div className="flex gap-2.5 min-w-0">
              <input
                ...
                className="flex-1 min-w-0 px-3 py-2 bg-stone-50 border border-stone-200 rounded-xl text-xs focus:outline-none"
              />
              <button
                type="button"
            ...
            <div className="flex flex-wrap gap-1.5">
              {materials.map((m, idx) => (
                <span
                  key={idx}
                  className="inline-flex items-center gap-1.5 bg-stone-50 border border-stone-150 px-2.5 py-1 rounded-full text-[10px] font-mono text-stone-600"
                >
                  <span>{m}</span>
                  <button
                    type="button"
          ...
          <div className="bg-white p-5 sm:p-6 border border-stone-200 rounded-2xl sm:rounded-3xl space-y-4">
            ...
            <div className="space-y-2">
              {colors.map((c, idx) => (
                <div
                  key={idx}
                  className="flex justify-between items-center gap-2 bg-stone-50 p-2 border border-stone-150 rounded-lg text-xs"
                >
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      setColors(colors.filter((item) => item.name !== c.name))
                    }
          ...
          <div className="bg-white p-5 sm:p-6 border border-stone-200 rounded-2xl sm:rounded-3xl space-y-4">
            ...
            <div className="space-y-2">
              <p className="text-[11px] text-stone-400 font-light leading-normal">
                Input high-definition photography URLs (e.g. Unsplash images).
              </p>
            ...
            <div className="space-y-2.5">
              {images.map((img, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-between gap-2 p-2 hover:bg-stone-50 border border-stone-100/50 rounded-lg text-xs min-w-0"
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="text-stone-500 hover:underline hover:text-stone-900 truncate max-w-[180px] md:max-w-[140px]"
                    >
                      {img}
                    </a>
                  </div>
          ...
          >
            ...
            <span>{isEdit ? "Re-lock Blueprint" : "Add new product"}</span>
          </button>
        </div>
      </form>
    </div>
