...
export default function SellerAnalytics() {
  ...
  const productTotal = Math.max(products.length, 1);
    ...
    <div className="font-sans text-stone-900 text-left space-y-6 sm:space-y-8">
      ...
      <div>
        <h1 className="text-xl sm:text-2xl font-display font-light text-stone-950 tracking-tight">
          Showroom Performance Analytics
        </h1>
        <p className="text-xs text-stone-400 mt-1">
          Review monthly SWIFT revenue, category spreads, and timber warranty
          commission rates.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
        ...
        <div className="lg:col-span-2 bg-white border border-stone-200 rounded-2xl sm:rounded-3xl p-5 sm:p-8 space-y-6 sm:space-y-8">
          <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-3 pb-3 border-b border-stone-100">
            <div className="min-w-0">
              ...
              <p className="text-[11px] text-stone-400 font-sans">
                Monthly performance including Sandbox wire orders.
              </p>
            </div>

            <span className="self-start text-[10px] bg-emerald-50 text-emerald-700 border border-emerald-150 px-2 py-1 rounded uppercase font-mono font-bold flex items-center gap-1 whitespace-nowrap">
          ...
          <div className="space-y-6 pt-4">
            <div className="flex items-end justify-between h-48 px-1 sm:px-4 border-b border-stone-150 relative">
              {monthlyData.map((data, idx) => {
                const heightPercentage = Math.floor(
                  (data.sales / maxVal) * 100,
                );
                return (
                  <div
                    key={idx}
                    className="flex h-full flex-col items-center justify-end w-1/5 group"
                  >
                    ...
                    <div
                      className="w-7 sm:w-12 bg-[#c2ab77] hover:bg-stone-900 rounded-t-lg transition-all"
                      style={{
                    ...
                    <span className="font-mono text-[10px] text-stone-400 mt-3">
                      {data.month}
                    </span>
                  </div>
                );
            ...
            <div className="text-stone-500 text-[10px] font-mono flex items-center gap-2 bg-stone-50 border border-stone-100 p-3 rounded-xl leading-relaxed">
              ...
              <span>
                Sales levels track commission payments cleared. Live updates
                refresh as buyers confirm and authorize checkout ledger items.
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-1 bg-white border border-stone-200 rounded-2xl sm:rounded-3xl p-5 sm:p-8 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between font-mono text-[10px] text-stone-600">
                ...
                <span>{categoriesCount["living-room"]} products</span>
              </div>
              <div className="h-2 bg-stone-100 rounded-full overflow-hidden">
                <div
                  className="h-full bg-stone-950 rounded-full"
                  style={{
                    width: `${(categoriesCount["living-room"] / productTotal) * 100}%`,
                  }}
                />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between font-mono text-[10px] text-stone-600">
                ...
                <span>{categoriesCount["dining"]} products</span>
              </div>
              <div className="h-2 bg-stone-100 rounded-full overflow-hidden">
                <div
                  className="h-full bg-[#c2ab77] rounded-full"
                  style={{
                    width: `${(categoriesCount["dining"] / productTotal) * 100}%`,
                  }}
                />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between font-mono text-[10px] text-stone-600">
                ...
                <span>{categoriesCount["bedroom"]} products</span>
              </div>
              <div className="h-2 bg-stone-100 rounded-full overflow-hidden">
                <div
                  className="h-full bg-stone-700 rounded-full"
                  style={{
                    width: `${(categoriesCount["bedroom"] / productTotal) * 100}%`,
                  }}
                />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between font-mono text-[10px] text-stone-600">
                ...
                <span>{categoriesCount["office"]} products</span>
              </div>
              <div className="h-2 bg-stone-100 rounded-full overflow-hidden">
                <div
                  className="h-full bg-stone-400 rounded-full"
                  style={{
                    width: `${(categoriesCount["office"] / productTotal) * 100}%`,
                  }}
                />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between font-mono text-[10px] text-stone-600">
                ...
                <span>{categoriesCount["lighting"]} products</span>
              </div>
              <div className="h-2 bg-stone-100 rounded-full overflow-hidden">
                <div
                  className="h-full bg-[#e4cfa1] rounded-full"
                  style={{
                    width: `${(categoriesCount["lighting"] / productTotal) * 100}%`,
                  }}
                />
              </div>
            </div>
          ...
          <div className="pt-4 border-t border-stone-100 text-[10px] font-mono text-stone-400">
            Total active registered catalog assets:{" "}
            <b className="text-stone-700">{products.length} products</b>
          </div>
        </div>
      </div>
    </div>
