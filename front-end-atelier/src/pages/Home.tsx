...
export default function Home() {
  ...
  const handleProtectedAction = (targetPath: string) => {
    if (!user) {
      // Save state to allow redirect back after successful login
    ...
    <div className="font-sans text-stone-900 bg-white">
        ...
        <div className="relative max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          <div className="space-y-8 text-left max-w-2xl">
            ...
            <h1 className="font-display font-light text-5xl sm:text-6xl lg:text-[4.25rem] leading-[1.1] tracking-tight text-white">
              Sovereign Craft. <br />
              <span className="font-normal text-[#c2ab77]">Uncompromising</span> <br />
              Interiors.
            </h1>
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              <button
                type="button"
                onClick={() => handleProtectedAction('/customer/products')}
                className="px-10 py-5 bg-[#c2ab77] hover:bg-[#aa935c] text-white text-xs font-mono font-bold tracking-widest uppercase rounded-lg transition-all hover:shadow-xl hover:shadow-amber-950/20 active:scale-98 flex items-center justify-center gap-2 group cursor-pointer"
              >
              ...
              <button
                type="button"
                onClick={() => handleProtectedAction('/customer/products/nordic-oak-table')}
                className="px-8 py-5 bg-stone-900 hover:bg-stone-850 text-white border border-stone-800 text-xs font-mono font-bold tracking-widest uppercase rounded-lg transition-all hover:shadow-lg active:scale-98 flex items-center justify-center gap-2 cursor-pointer"
              >
            ...
            <div className="grid grid-cols-3 gap-8 pt-10 border-t border-stone-900 font-mono text-stone-400 text-xs">
              <div>
                ...
                <p className="mt-1 font-mono text-[9px] uppercase tracking-wider text-stone-550">FSC CERTIFIED TIMBER</p>
              </div>
              <div>
                <p className="text-white text-xl font-display font-medium">0.02%</p>
                <p className="mt-1 font-mono text-[9px] uppercase tracking-wider text-stone-550">ZERO CARBON WASTAGE</p>
              </div>
              <div>
                <p className="text-white text-xl font-display font-medium">EST.</p>
                <p className="mt-1 font-mono text-[9px] uppercase tracking-wider text-stone-550">INDIVIDUAL COMMISSION CODE</p>
              </div>
            </div>
          ...
          <div className="relative aspect-[4/5] sm:aspect-square lg:aspect-[4/5] bg-stone-900/40 rounded-2xl border border-stone-850 overflow-hidden flex items-center justify-center p-8 group">
            ...
            <div className="relative z-10 w-full flex flex-col items-center justify-center text-center space-y-4">
              ...
              <div className="bg-stone-950/70 border border-stone-800 p-5 rounded-xl max-w-sm text-left shadow-2xl backdrop-blur-md">
                <span className="text-[9px] font-mono tracking-widest text-[#B39D69] uppercase font-bold">SEASON HEIRLOOM FEATURE</span>
                ...
                <div className="mt-3.5 flex justify-between items-center">
                  ...
                  <button 
                    type="button"
                    onClick={() => handleProtectedAction('/customer/products/jeanneret-easy-chair')}
                    className="text-[10px] font-mono text-[#c2ab77] hover:text-white uppercase tracking-wider flex items-center gap-1 group/btn cursor-pointer"
                  >
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
        <div className="text-center max-w-2xl mx-auto mb-16 space-y-3">
          <span className="text-[10px] bg-stone-100 text-stone-750 font-mono font-bold tracking-widest uppercase px-3 py-1 rounded-md">Our Manifesto</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 font-sans">
          ...
          <div className="p-8 bg-stone-50 border border-stone-100 rounded-2xl flex flex-col justify-between group hover:border-stone-300 transition-all duration-300">
      ...
      <section className="bg-stone-50/50 py-24 border-y border-stone-150">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-16">
            ...
            <button
              type="button"
              onClick={() => handleProtectedAction('/customer/products')}
              className="px-5 py-3 border border-stone-200 hover:border-stone-900 rounded-lg text-xs font-mono font-bold tracking-wider uppercase transition-all flex items-center gap-2 group cursor-pointer bg-white shadow-xs"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 text-left">
            {featuredProducts.map((p) => (
              <div 
                key={p.id}
                onClick={() => handleProtectedAction(`/customer/products/${p.id}`)}
                className="group bg-white rounded-2xl border border-stone-200 overflow-hidden cursor-pointer shadow-xs hover:shadow-xl hover:border-stone-400 transition-all duration-500 flex flex-col justify-between"
              >
                {/* Image */}
                <div className="relative aspect-square w-full bg-stone-50 p-6 flex items-center justify-center border-b border-stone-100">
                  ...
                  <img
                    ...
                    alt={p.name}
                    referrerPolicy="no-referrer"
                    className="max-h-[90%] max-w-[90%] object-contain drop-shadow transition-transform duration-500 group-hover:scale-105 pointer-events-none"
                  />
                ...
                <div className="p-6 space-y-4 flex-1 flex flex-col justify-between">
                  <div className="space-y-2">
                    <div className="flex justify-between items-start gap-4">
                      <h3 className="font-display font-medium text-base text-stone-900 group-hover:text-amber-700 transition-[#c2ab77]">
                        {p.name}
                      </h3>
